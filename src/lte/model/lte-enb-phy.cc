...
//////////////////////////////////// MODIFIED ////////////////////////////////////
     .AddTraceSource ("AbsCycle",
    		 	 	 "Reports ABS Change policy",
                      MakeTraceSourceAccessor (&LteEnbPhy::absCycle),
...
LteEnbPhy::StartSubFrame (void)
{
  ...
  if (m_channelAccessManager == 0)
      ...
      else
	{
	  ...
	  if (!IsNonEmptyCtrMessage() && !IsNonEmptyCtrMessage())
	    {
	      GetControlMessages ();
	      GetPacketBurst();
	    }
	  ...
	  /*else if (drsMessageReady && ((m_ttiBegin - m_lastDrsMessageSent) >= m_drsPeriod))  // there is drs message ready to be sent
	      ...
	      if (m_grantTimeout > m_ttiBegin)
		{
		  ...
		  if ((IsNonEmptyPacketBurst() && m_grantTimeout-m_ttiBegin < GetTti()) ||
	      ...
	      else
		{
		  ...
		  if (m_grantTimeout - m_ttiBegin >=  Seconds(GetTti()))
		    ...
		    {
		      if ((m_dropPackets) && (IsNonEmptyPacketBurst() || IsNonEmptyCtrMessage()))
			{
			  GetControlMessages();
			  GetPacketBurst();
			}
...
LteEnbPhy::TransmitSubFrame (void)
{
  ...
  if (isAlmostBlankSubframe) // Almost Blank Subframe
	...
	if (m_channelAccessManager) // check if to send reservation signal after ctrl
	  {
...
LteEnbPhy::DoSendReservationSignal ()
{
  ...
  NS_ASSERT_MSG(m_channelAccessManager, "When m_channelAccessManager is not set, SendReservationSingal function should not be called.");
...
/////////////////////////// MODIFIED ////////////////////////////////
void
LteEnbPhy::DoSetAbsPatternDouble (double absPattern){
	m_absPattern = calcAbsPattern(absPattern);
}

std::bitset<40>
LteEnbPhy::calcAbsPattern(double lteDutyCycle){
	std::bitset<40> absPattern;
	  ...
	  //std::cout << "New LTE duty cycle: requested " << lteDutyCycle << ", actual " << actualLteDutyCycle << ", ABS pattern " << absPattern << std::endl;
	  return absPattern;
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG(m_channelAccessManager!=0, "Channel access manager not set!");
