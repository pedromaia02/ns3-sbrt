...
int GetNodeFromContext(std::string s){
	...
	std::cout << "Error: Context not found" << std::endl;
	return -1;
}

void TxEnbDataSent(std::string context, uint32_t newval){
	int i_eNB = GetNodeFromContext(context);
	for (std::list<BsQLearning>::iterator i = bsQlearningList.begin(); i != bsQlearningList.end(); i++){
		if (i->GetNode() == i_eNB){
			i->SaveEnbTxData(newval);
		}
	}
}

void TxApDataSent(std::string context, Ptr<const Packet> packet){
	int node = GetNodeFromContext(context);

	//Wifi::phy also calls TX from STAs. So we need to confirm it's an AP.
	for (std::list<uint32_t>::iterator i = apList.begin(); i != apList.end(); i++){
		  if ((int)*i == node){
			  //Then we set the buffer of all eNBs to be aware of Wi-Fi.
			  for (std::list<BsQLearning>::iterator j = bsQlearningList.begin(); j != bsQlearningList.end(); j++){
				  j->SaveApTxData(packet->GetSize());
			  }
		  }
	  }
}
...
{
  Config::SetDefault ("ns3::LteRlcUm::MaxTxBufferSize", UintegerValue (2000000)); // adicionei para ver se muda o Tput do LTE (e realmente aumenta Kkk)
                         ...
                         std::string simulationParams)
{
  ...
  if (activated){
	  ...
	  for (NodeContainer::Iterator it = bsNodesA.Begin (); it != bsNodesA.End (); ++it){
		  BsQLearning *eNb = new BsQLearning();
		  eNb->SetNode((*it)->GetId());
		  bsQlearningList.push_back(*eNb);
	  }

	  /*  Wi-Fi : bsNodesB  */
	  for (NodeContainer::Iterator it = bsNodesB.Begin (); it != bsNodesB.End (); ++it){
		  apList.push_back((*it)->GetId());
	  }
  }
////////////////////////////////////////////////////////////////////////
 ...
 Ptr<CwStatistics> lteCwStatistics = CreateObject<CwStatistics>();
  ...
  //
  if (activated){
	  Config::Connect ("/NodeList/*/DeviceList/*/$ns3::LteNetDevice/$ns3::LteEnbNetDevice/LteEnbPhy/DataSent", MakeCallback (&TxEnbDataSent));
