...
Time
WifiPhy::GetPlcpSyncFieldDuration (WifiMode payloadMode, WifiPreamble preamble, WifiTxVector txVector)
{
  if (preamble == WIFI_PREAMBLE_NONE)
  {
      return MicroSeconds(0);
    ...
    case WIFI_MOD_CLASS_ERP_OFDM:
      {
        ...
        double numDataBitsPerSymbol = payloadMode.GetDataRate (txVector.GetChannelWidth (), 0, txVector.GetNss ()) * symbolDuration.GetNanoSeconds () / 1e9;
        double numSymbols;
    ...
    case WIFI_MOD_CLASS_VHT:
      {
        ...
        double numSymbols;
#if 0
  // XXX the multiplication by Nss() is questionable below
        double numDataBitsPerSymbol = payloadMode.GetDataRate (txVector.GetChannelWidth (), txVector.IsShortGuardInterval (), txVector.GetNss ()) * txVector.GetNss () * symbolDuration.GetNanoSeconds () / 1e9;
#endif
...
uint32_t
WifiPhy::GetNssForHtMcs (WifiMode mcs)
{
  if (mcs == WifiPhy::GetHtMcs8 () || mcs == WifiPhy::GetHtMcs9 () ||
      mcs == WifiPhy::GetHtMcs10 () || mcs == WifiPhy::GetHtMcs11 () ||
      mcs == WifiPhy::GetHtMcs12 () || mcs == WifiPhy::GetHtMcs13 () ||
      mcs == WifiPhy::GetHtMcs14 () || mcs == WifiPhy::GetHtMcs15 ())
    {
      return 2;
    }
  else if (mcs == WifiPhy::GetHtMcs0 () || mcs == WifiPhy::GetHtMcs1 () ||
      mcs == WifiPhy::GetHtMcs2 () || mcs == WifiPhy::GetHtMcs3 () ||
      mcs == WifiPhy::GetHtMcs4 () || mcs == WifiPhy::GetHtMcs5 () ||
      mcs == WifiPhy::GetHtMcs6 () || mcs == WifiPhy::GetHtMcs7 ())
    {
      return 1;
    }
  else
    {
      NS_LOG_WARN ("Not an HT MCS" << mcs);
      return 0;
    }
}
