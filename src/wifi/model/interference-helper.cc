...
InterferenceHelper::CalculateSnr (double signal, double noiseInterference, uint32_t channelWidth) const
{
  ...
  NS_LOG_DEBUG ("bandwidth(MHz)=" << channelWidth << ", signal(W)= " << signal << ", noise(W)=" << noiseFloor << ", interference(W)=" << noiseInterference << ", snr(linear)=" << snr << " snr(db)=" << WifiPhy::RatioToDb (snr));
  return snr;
}
...
{
  NS_LOG_FUNCTION (this << snir << duration << mode << txVector);
  if (duration == NanoSeconds (0))
    {
      return 1.0;
    }
  uint32_t rate = mode.GetPhyRate (txVector.GetChannelWidth (), txVector.IsShortGuardInterval (), txVector.GetNss());
...
InterferenceHelper::CalculatePlcpPayloadPer (Ptr<const InterferenceHelper::Event> event, NiChanges *ni) const
{
  ...
  double per = 1 - psr;
  NS_LOG_DEBUG ("Returning per: " << per);
  return per;
}
...
InterferenceHelper::CalculateAvgSyncFieldSnir (Ptr<InterferenceHelper::Event> event)
{
  ...
  Time plcpSyncFieldEnd = packetStartTime + WifiPhy::GetPlcpSyncFieldDuration (payloadMode, preamble, txVector); // packet start time + sync field
