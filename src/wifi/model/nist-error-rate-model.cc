...
NistErrorRateModel::GetChunkSuccessRate (WifiMode mode, WifiTxVector txVector, double snr, uint32_t nbits) const
{
  ...
  if (mode.GetModulationClass () == WIFI_MOD_CLASS_ERP_OFDM
    ...
    {
      if (mode.GetConstellationSize (txVector.GetNss ()) == 2)
        {
          if (mode.GetCodeRate (txVector.GetNss ()) == WIFI_CODE_RATE_1_2)
            {
      ...
      else if (mode.GetConstellationSize (txVector.GetNss ()) == 4)
        {
          if (mode.GetCodeRate (txVector.GetNss ()) == WIFI_CODE_RATE_1_2)
            {
      ...
      else if (mode.GetConstellationSize (txVector.GetNss ()) == 16)
        {
          if (mode.GetCodeRate (txVector.GetNss ()) == WIFI_CODE_RATE_1_2)
            {
      ...
      else if (mode.GetConstellationSize (txVector.GetNss ()) == 64)
        {
          if (mode.GetCodeRate (txVector.GetNss ()) == WIFI_CODE_RATE_2_3)
            {
              return GetFec64QamBer (snr,
                                     nbits,
                                     2); //b value
            }
          else if (mode.GetCodeRate (txVector.GetNss ()) == WIFI_CODE_RATE_5_6)
            {
              return GetFec64QamBer (snr,
                                     nbits,
                                     5); //b value
      ...
      else if (mode.GetConstellationSize (txVector.GetNss ()) == 256)
        {
          if (mode.GetCodeRate (txVector.GetNss()) == WIFI_CODE_RATE_5_6)
            {
