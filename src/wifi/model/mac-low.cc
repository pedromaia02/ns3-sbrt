...
MacLow::ReceiveError (Ptr<const Packet> packet, double rxSnr, bool isEndOfFrame)
{
  ...
  // Bug 2185:  additional processing disabled
  return;
}
...
MacLow::SendAckAfterData (Mac48Address source, Time duration, WifiMode dataTxMode, double dataSnr)
{
  ...
  if (duration < MicroSeconds (0))
    {
      // This issue arises if the duration picked by the sender for virtual
      // carrier sense, based on a guess of the WiFi mode that will be used
      // for the ack, is not aligned with the actual WiFi mode picked for
      // the ack.  This is an upstream ns-3-dev problem and we can just
      // ignore it here since it is sufficient to set duration 0 on acks
      NS_LOG_WARN ("duration was negative: " << duration.GetMicroSeconds ());
      duration = MicroSeconds (0); // No need to set non-zero duration in ack
    }
                              ...
                              Time duration, WifiMode blockAckReqTxMode, double rxSnr)
{
  ...
  if (immediate)
    {
      ...
      duration = MicroSeconds (0); // No need to set non-zero duration in
                                   // immediate block ack response
    }
  else
    {
      m_txParams.EnableAck ();
  ...
  // Apply SNR tag for ideal rate control
  SnrTag tag;
  tag.Set (rxSnr);
  packet->AddPacketTag (tag);
  NS_LOG_DEBUG ("SnrTag data rxSnr in Block Ack Response:  " << rxSnr);
