...
public:
   ...
   *
   * \return the duration of the PLCP SYNC field
   */
  static Time GetPlcpSyncFieldDuration (WifiMode payloadMode, WifiPreamble preamble, WifiTxVector txVector);
  ...
  /**
   * \param txVector the transmission mode
  ...
  */
  ...
  /**
   * In general, the number of spatial streams is not encoded in
   * the WifiMode (e.g. the VHT MCSes).  However, for HT MCS, the
   * MCS index can be used to determine the number of spatial streams.
   *
   * \param mcs the WifiMode to check
   * \return the number of spatial streams; zero if the WifiMode is not an HtMcs
   */
  static uint32_t GetNssForHtMcs (WifiMode mcs);
